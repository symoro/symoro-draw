...
def create_arrow_array(length, thick_rod=0.04, thick_hat=0.125, length_hat=1.3):
    ...
    for i, j in product([-1, 1], repeat=2):
        vertices += (0, i*thick_rod*length, i*j*thick_rod*length)
        vertices += (length, i*thick_rod*length, i*j*thick_rod*length)
        normals += 2 * (0, 0.707*i, 0.707*i*j)
        
    # Arrow head
    for i, j in product([-1, 1], repeat=2):
        vertices += (length, i*thick_hat*length, i*j*thick_hat*length)
        normals += (0, 0.707*i, 0.707*i*j)
    vertices += (length_hat * length, 0, 0)
    normals += (0, 0, 1)
    for i in range(8):
